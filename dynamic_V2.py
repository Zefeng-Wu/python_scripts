...
for idx_row in range(1,n+1):
    for idx_col in range(1,m+1):
        ...
        # record best path
        three_scores = [match_path_score,horit_path_score,verti_path_socre]
        best_indexes = [i for i, x in enumerate(three_scores) if x == max(three_scores)]
        if 0 in best_indexes:
            trace_matrix.iloc[idx_row,idx_col] += "match"
        if 1 in best_indexes:
            trace_matrix.iloc[idx_row,idx_col] +="left_gap"
        if 2 in best_indexes:
